...
| 1 — Thin handoff | Supervisor | Short `docs/handoffs/current.md`: Intent, acceptance, API/DB contract, GitHub tracking |
...
| `db`…`docs` | 2 | `developer-implementer` Task | Supervisor in-chat |
...
1. Short `docs/handoffs/current.md` (Intent, acceptance, contracts, GitHub tracking).
...
```text
You are running as the specifier subagent for Newsroom.
...
2. Implement db→docs via developer-implementer checklist.
...
```text
You are running as the developer-implementer subagent for Newsroom.
...
1. load-feature-issue.sh for open db…docs tasks.
