...
#   ./scripts/load-feature-issue.sh prog-clarity-flash
#   ./scripts/load-feature-issue.sh "clarity flash"
#   ./scripts/load-feature-issue.sh --json prog-clarity-flash
#
...
REPO="${GITHUB_REPO:-SpektrNO/newsroom}"
OWNER="${REPO%%/*}"
NAME="${REPO#*/}"
JSON=false
...
TASK_ORDER=(audit spec db api worker web mobile verify docs)
...
resolve_issue_number() {
  ...
  echo "hint: use issue number, feature id (e.g. prog-clarity-flash), or distinctive title words" >&2
  exit 1
}
...
order = ['audit', 'spec', 'db', 'api', 'worker', 'web', 'mobile', 'verify', 'docs']
...
Implement open tasks in this order: \`audit\` → \`spec\` → \`db\` → \`api\` → \`worker\` → \`web\` → \`mobile\` → \`verify\` → \`docs\`
(section-specific features may omit some slugs — skip missing tasks)
