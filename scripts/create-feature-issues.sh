...
task_list_for() {
  ...
  case "$section" in
    *foundation*|*infra*)
      tasks+=("$TASK_DB" "$TASK_API" "$TASK_VERIFY" "$TASK_DOCS")
      ;;
    *ingest*|*rank*)
      tasks+=("$TASK_DB" "$TASK_API" "$TASK_WORKER" "$TASK_VERIFY" "$TASK_DOCS")
      ;;
    *web*)
      tasks+=("$TASK_API" "$TASK_WEB" "$TASK_VERIFY" "$TASK_DOCS")
      ;;
    *mobile*)
      tasks+=("$TASK_API" "$TASK_MOBILE" "$TASK_VERIFY" "$TASK_DOCS")
      ;;
    *)
      tasks+=("$TASK_DB" "$TASK_API" "$TASK_WORKER" "$TASK_WEB" "$TASK_MOBILE" "$TASK_VERIFY" "$TASK_DOCS")
      ;;
  esac
...
build_parent_body() {
  ...
  done < <(task_list_for "$status" "${6:-foundation}")
...
EOF
    ...
    ((tasks_created++)) || true
    sleep 0.3
  done < <(task_list_for "$status" "$section")
done < "$FEATURE_ROWS_FILE"
