...
/**
 * Small local models sometimes echo the prompt's own instructions, schema
 * placeholders, or internal ids instead of describing the article (e.g.
 * "Candidate topics fully match confirmed topic ids…", "matches multiple
 * candidateTopics including t0, t6…", or literally "one line" — the JSON
 * schema's own placeholder value for `reason`). Treat that as no reason and
 * fall back to the keyword match.
 */
...
/** Exact (punctuation/case-insensitive) placeholder echoes, not substrings. */
const BOILERPLATE_REASON_EXACT = new Set(["one line", "fill in", "see below"]);
...
function isBoilerplateReason(reason: string): boolean {
  ...
  const bare = reason.trim().toLowerCase().replace(/[.!]+$/, "");
  if (BOILERPLATE_REASON_EXACT.has(bare)) return true;
